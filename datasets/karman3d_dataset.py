...
from le_pde.utils import PDE_PATH, KARMAN3D_PATH
from le_pde.pytorch_net.util import plot_matrices
from le_pde.utils import get_root_dir, to_tuple_shape
from le_pde.utils import read_zipped_array, write_zipped_array
...
if __name__ == "__main__":
    ...
    # space:
    for tailin in range(10,200,1):
        data = dataset[0]
        x = data.x
        x_reshape = x[...,2].reshape(256, 128, 128)
        x = data.x
        x_reshape = x[...,1].reshape(256, 128, 128)
        plt.imshow(x_reshape[:,:,tailin], cmap="jet")
        plt.show()
    
    # time:
    for tailin in range(10,200,80):
        data = dataset[tailin]
        x = data.x
        x_reshape = x[...,1].reshape(256, 128, 128)
        plt.imshow(x_reshape[:,:,60], cmap="jet")
